...
- Local indexes live under `lancedb/<namespace>/`. Generated pipeline data and evaluation traces live under `data_pipelines/data/` and `environments/ragent_deep_search/outputs/` respectively.
...
The currently tracked automated tests cover the model services:
...
Common variables include `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `PRIME_API_KEY`, `HF_TOKEN`, `FIREWORKS_ACCOUNT_ID`, `LANCEDB_DB_URI`, `LANCEDB_GCS_URI`, `GCS_SERVICE_ACCOUNT`, `RAGENT_EMBEDDING_SERVICE_URL`, `RAGENT_RERANKER_SERVICE_URL`, and `RAGENT_RETRIEVER_AUTHKEY`. Keep local service-account files under ignored paths such as `secrets/`.
