...
def find_text_dir(root_dir: str) -> str:
    default_path = os.path.join(root_dir, "text")
    ...
    raise FileNotFoundError("Could not locate text directory in cloned repo")
...
def prepare_dataset() -> None:
    ...
    output_path = Path("data/rust_rfcs")
    output_path.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory() as temp_dir:
        clone_repo(repo_url, temp_dir)
        content_dir = find_text_dir(temp_dir)
        ...
        dataset = Dataset.from_pandas(df, info=DatasetInfo(dataset_name="rust_rfcs"))
