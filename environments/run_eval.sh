...
ENV=${2:-bm25} # Extracting environment from "environments.bm25"
echo "Running evaluation for $MODEL in $ENV"

source ${ENV}/.venv/bin/activate
vf-eval ${ENV} --endpoints-path ../ragent_core/ragent_core/config/endpoints.py --model $MODEL --max-tokens 32768 --save-dataset --verbose
